...
Read the data:
```{r}
raw_data = read_csv("results-2.csv")
glimpse(raw_data)
```  
...
data = rename(raw_data,
  submissionID = `submission+AF8-id`,
  trialName = `trial+AF8-name`
)
...
Calculate age mean, age range and gender distribution:
```{r}
...
Pearon correlation of liking and detectability: 
```{r}
cor(as.numeric(d$liking), as.numeric(d$detectability))
```

M1. simple analysis: "objects" ratings treated interval scale / metric
...
m4 = brm(formula = liking ~ mo(detectability)*artist*color*color2 + (1|submissionID), 
         data = d,
         family = "cumulative")
...
m5 = brm(formula = liking ~ mo(detectability)*artist*color*color2, 
          data = d,
          family = "cumulative")
